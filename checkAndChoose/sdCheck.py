...
import csv
filename = "./zip/sd_20231009.csv"  # 替换为你的CSV文件名
...
# all_numbers = set(range(10)) # 创建一个包含0到80的集合
...
for i, number in enumerate(cj):
    # 遍历集合
    for mapKey, mapValue in ckMap.items():
        if number in mapValue:
            position = mapValue.index(number)
            if i not in result:
                result[i] = {}
            result[i][mapKey] = position
